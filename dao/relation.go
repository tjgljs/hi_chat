...
// 获取所有好友的聊天记录
func GetMysqlMessages(userId uint) (*[]models.Message, error) {
	Friends, err := FriendList(userId)
	if err != nil {
		fmt.Printf("err: %v\n", err)
	}
	Ids := make([]uint, 0)
	...
	err = global.DB.Model(new(models.Message)).Where("form_id in ? AND target_id= ?", Ids, userId).Find(&messages).Error
	if err != nil {
		return nil, err
	}
	return &messages, nil
